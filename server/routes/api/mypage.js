...
router.get('/', async ctx => {
  const member = await Member.query()
    ...
    .select('id', 'username', 'name', 'email', 'phone', 'flexUsername', 'createdAt')
...
router.patch('/', async ctx => {
  ...
  ctx.body = {
    code: 200,
    data: { id: updated.id, username: updated.username, name: updated.name, email: updated.email, phone: updated.phone, flexUsername: updated.flexUsername }
  }
})
...
router.patch('/password', async ctx => {
  ...
  if (!currentPassword || !newPassword) {
    ctx.status = 400
    ctx.body = { code: 400, message: '현재 비밀번호와 새 비밀번호를 입력해주세요.' }
    return
  }
