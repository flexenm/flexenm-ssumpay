...
router.post('/password/reset', authLimiter, async ctx => {
  ...
  if (member) {
    ...
    await PasswordResetToken.query().delete().where({ memberId: member.id }).whereNull('usedAt')
  ...
  ctx.body = { code: 200, message: '입력하신 이메일로 비밀번호 재설정 링크를 발송했습니다.' }
})
...
router.post('/password/reset/confirm', authLimiter, async ctx => {
  ...
  if (!row) {
    ctx.status = 400
    ctx.body = { code: 400, message: '유효하지 않거나 만료된 링크입니다. 다시 요청해주세요.' }
    return
  }
  ...
  await PasswordResetToken.transaction(async trx => {
    ...
    if (marked === 0) {
      const err = new Error('유효하지 않거나 만료된 링크입니다. 다시 요청해주세요.')
      err.status = 400
      throw err
    }

    await Member.query(trx).patchAndFetchById(row.memberId, { password: hashed })
  })

  ctx.body = { code: 200, message: '비밀번호가 변경되었습니다. 새 비밀번호로 로그인해주세요.' }
})
